    ...
    this._handlePickedFile = async (file) => {
      try {
        ...
        if (this.ws && this.ws.readyState === WebSocket.OPEN) {
          console.log(
            "Interrupting and sending image (chunked) to server WebSocket"
          );
          ...
          this.ws.send(
            JSON.stringify({ type: "image_start", id, text: promptText })
          );
          ...
          console.warn(
            "Not connected; image will not be sent. Click Connect first."
          );
        }
